...
async function fixture(t) {
  const root = await mkdtemp(join(tmpdir(), "session-rename-retirement-"))
...
test("deployment retires managed rename artifacts and registrations without touching user data", async (t) => {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    assert.deepEqual(JSON.parse(await readFile(join(targetRoot, "opencode.json"), "utf8")), preservedConfig)
    assert.deepEqual(JSON.parse(await readFile(join(targetRoot, "tui.json"), "utf8")), {
      theme: "preserved", plugin: [...unrelatedPlugins, ...deployedSpecs],
    })
    const current = await snapshot(targetRoot)
    assert.deepEqual(Object.keys(current).sort(), ["tui.json", "opencode.json", "opencode-tools-shared.js", ...deployedSpecs.map((spec) => spec.slice(2))].sort())
...
test("deployment preserves distinct custom rename commands and title-agent preferences", async (t) => {
  ...
  for (const { name, command } of cases) {
    await t.test(name, async (t) => {
      ...
      const config = { command: { "session-rename": command }, agent: { title: { disable: false } } }
      await writeFile(join(targetRoot, "opencode.json"), JSON.stringify(config))
      await deployPlugins(targetRoot, { logLevel: "silent" })
      assert.deepEqual(JSON.parse(await readFile(join(targetRoot, "opencode.json"), "utf8")), config)
    })
  }
})
...
test("deployment removes the generated-only command without creating title preferences", async (t) => {
  const { targetRoot } = await fixture(t)
  await writeFile(join(targetRoot, "opencode.json"), JSON.stringify({ command: { "session-rename": generatedCommand } }))
  await deployPlugins(targetRoot, { logLevel: "silent" })
